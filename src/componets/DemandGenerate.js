...
const DemandGenerate = () => {
  ...
  const handleDemandedQuantityChange = (e, maxQuantity) => {
    ...
    if (value > maxQty) {
      setValidationError(`मांगी गई मात्रा (${value}) Champawat, का कुल लक्ष्य (${maxQty}) से अधिक नहीं हो सकती`);
      return;
    }
  ...
  const handleSaveDemand = async (demandId, allocatedQuantity) => {
    ...
    if (parseFloat(editingQuantity) > parseFloat(allocatedQuantity)) {
      setValidationError(`मांगी गई मात्रा Champawat, का कुल लक्ष्य (${allocatedQuantity}) से कम होनी चाहिए`);
      return;
    }
